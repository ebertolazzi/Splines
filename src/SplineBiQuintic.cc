...
namespace Splines {
  ...
  BiQuinticSpline::makeSpline() {
    ...
    for ( integer j = 0; j < ny; ++j ) {
      ...
      for ( integer i = 0; i < nx; ++i ) {
        DXY[size_t(ipos_C(i,j))]   += sp.ypNode(i);   DXY[size_t(ipos_C(i,j))]   /= 2;
        DXXY[size_t(ipos_C(i,j))]  += sp.yppNode(i);  DXXY[size_t(ipos_C(i,j))]  /= 2;
        DXYY[size_t(ipos_C(i,j))]  += sp1.ypNode(i);  DXYY[size_t(ipos_C(i,j))]  /= 2;
        DXXYY[size_t(ipos_C(i,j))] += sp1.yppNode(i); DXXYY[size_t(ipos_C(i,j))] /= 2;
      }
    }
